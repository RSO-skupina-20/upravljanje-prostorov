...
public class ProstoriVir {
    ...
    @APIResponses({
            @APIResponse(responseCode = "200", description = "Seznam prostorov", content = @Content(schema = @Schema(implementation = Prostor.class)),
                    headers = @Header(name = "X-Total-Count", description = "Število vrnjenih prostorov", schema = @Schema(type = SchemaType.INTEGER))),
            @APIResponse(responseCode = "404", description = "Prostori ne obstajajo", content = @Content(mediaType = "application/json", schema = @Schema(implementation = String.class, example = "{\"napaka\": \"Prostorov ni mogoče najti\"}"))),
    })
    ...
    @APIResponses({
            @APIResponse(responseCode = "201", description = "Prostor uspešno dodan", content = @Content(schema = @Schema(implementation = Prostor.class))),
            @APIResponse(responseCode = "400", description = "Manjkajo obvezni podatki", content = @Content(mediaType = "application/json", schema = @Schema(implementation = String.class, example = "{\"napaka\": \"Manjkajo obvezni podatki\"}"))),
            @APIResponse(responseCode = "404", description = "Uporabnik ne obstaja", content = @Content(mediaType = "application/json", schema = @Schema(implementation = String.class, example = "{\"napaka\": \"Uporabnik ne obstaja\"}"))),
    })
    public Response dodajProstor(@RequestBody(description = "Prostor za dodajanje", required = true, content = @Content(schema =
        @Schema(implementation = Prostor.class, example = "{\"ime\": \"Ime\", \"lokacija\": \"Lokacija\", \"cena\": 100.0, \"velikost\": 100, \"opis\": \"Opis\", \"lastnik\": 1}"))) Prostor prostor){

        if(prostor.getIme() == null || prostor.getLokacija() == null || prostor.getCena() == null || prostor.getLastnik() == null || prostor.getOpis() == null){
            return Response.status(Response.Status.BAD_REQUEST).entity("{\"napaka\": \"Manjkajo obvezni podatki\"}").build();
        }

        prostor = prostorZrno.addProstor(prostor);
        if(prostor == null){
            // To je potrebno še dodati v zrno, da se preveri, če lastnik obstaja
            return Response.status(Response.Status.NOT_FOUND).entity("{\"napaka\": \"Uporabnik ne obstaja\"}").build();
        }
        return Response.status(Response.Status.CREATED).entity(prostor).build();
    }
